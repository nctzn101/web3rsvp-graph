...
export function handleNewEventCreated(event: NewEventCreated): void {
  // eventID used to identify Event entity
  let newEvent = Event.load(event.params.eventID.toHex()); // string to hex because eventID comes as string
  if (newEvent == null) { // if no such event exists, create it
    newEvent = new Event(event.params.eventID.toHex());
    // set values for each field from schema
    ...
    if (metadata) {
      ...
      if (value){
        ...
        // optional: imagePath + error handler
      }
    }
  newEvent.save();
  }
...
// get account info or create a new one
// handleNewEventCreated can also be split into two functions
...
export function handleConfrimedAttendee(event: ConfrimedAttendee): void {
